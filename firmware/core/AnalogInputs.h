...
#define ANALOG_INPUTS_FOR_ALL_PHY(iterator) for (AnalogInputs::Name iterator = AnalogInputs::Name(0); iterator < AnalogInputs::PHYSICAL_INPUTS_COUNT; iterator = AnalogInputs::Name(iterator + 1))
...
{
    AnalogInputs::Type getType(Name name)
    {
        switch (name)
        {
        case Iout:
    ...
    static const uint8_t PHYSICAL_INPUTS_COUNT = VirtualInputs - Vout_plus_pin;
