...
#define MADDR_REORDER(x) ((GET_BIT(x, 0) << (MUX_ADR0_PIN - 1)) + (GET_BIT(x, 1) << (MUX_ADR1_PIN - 1)) + (GET_BIT(x, 2) << (MUX_ADR2_PIN - 1)))
    ...
    const adc_channel order_analogInputs_on[] PROGMEM = {
    ...
    adc_channel adc_input_next;
    static volatile uint8_t collect_sum = 0;
    ...
    void _processConversion(uint16_t v)
    {
        ...
        if (name != AnalogInputs::VirtualInputs)
        {
            ...
            {
                AnalogInputs::adc_raw_values[name] = v;
            }
            if (collect_sum)
                AnalogInputs::adc_sum_values[name] += v;
        }
        else
        {
    ...
    {
        AnalogInputs::adc_raw_values[AnalogInputs::IsmpsSet] = SMPS::getValue();
        AnalogInputs::adc_raw_values[AnalogInputs::IdischargeSet] = Discharger::getValue();
        if (collect_sum)
        {
            AnalogInputs::adc_sum_values[AnalogInputs::IsmpsSet] += SMPS::getValue();
            AnalogInputs::adc_sum_values[AnalogInputs::IdischargeSet] += Discharger::getValue();
            if (AnalogInputs::i_avrCount_ == 1)
            {
                AnalogInputs::adc_sum_values[AnalogInputs::Ismps] /= ADC_STANDARD_PER_ROUND;
                AnalogInputs::adc_sum_values[AnalogInputs::Vout_plus_pin] /= ADC_STANDARD_PER_ROUND;
                AnalogInputs::adc_sum_values[AnalogInputs::Vout_minus_pin] /= ADC_STANDARD_PER_ROUND;
                AnalogInputs::adc_sum_values[AnalogInputs::Idischarge] /= ADC_STANDARD_PER_ROUND;
            }
...
#error "ANALOG_INPUTS_ADC_BURST_COUNT < 2"
    ...
    void onInterrupt()
    {
        ...
        case ANALOG_INPUTS_ADC_BURST_COUNT + 2:
            /* set next adc input */
            _setADCChannel(adc_input_next.adc);
            /* switch to new input */
    ...
    void _setupNextInput()
    {
        ...
        if (_currentInput == 0)
        {
            ...
            collect_sum = AnalogInputs::i_avrCount_ > 0;
        }
    }
