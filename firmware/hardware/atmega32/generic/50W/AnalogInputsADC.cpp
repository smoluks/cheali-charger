...
inline void _processConversion(uint16_t v)
{
...
inline void _finalizeMeasurement()
{
    ...
    if(collect_sum) {
        AnalogInputs::adc_sum_values[AnalogInputs::IsmpsSet]        += ANALOG_INPUTS_ADC_BURST_COUNT * SMPS::getValue();
        AnalogInputs::adc_sum_values[AnalogInputs::IdischargeSet]   += ANALOG_INPUTS_ADC_BURST_COUNT * Discharger::getValue();
        if(AnalogInputs::i_avrCount_ == 1) {
            AnalogInputs::adc_sum_values[AnalogInputs::Ismps]          /= ADC_I_SMPS_PER_ROUND;
        }
